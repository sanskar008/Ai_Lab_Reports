...
class ProblemSolver:
    ...
    def check_valid_transition(self, current_state, next_state):
        # Verify if the transition to the next state is valid
    ...
    def generate_successors(self, state):
        ...
        for move in possible_moves:
            ...
            if 0 <= new_index < len(state.state):
                ...
                if self.check_valid_transition(state, new_state):
                    successors.append(new_state)
        return successors

    def breadth_first_search(self):
        # Implement BFS to find the path from initial_state to target_state
